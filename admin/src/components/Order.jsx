...
const Order = () => {
    ...
    const fetchOrders = async () => {
      try {
        const response = await axios.get(
          "https://mern-food-delivery-webapplication-backend.onrender.com/api/orders/getall",
          {
            headers: {
  ...
  const handleStatusChange = async (orderId, newStatus) => {
    try {
      await axios.put(`https://mern-food-delivery-webapplication-backend.onrender.com/api/orders/getall/${orderId}`, {
        status: newStatus,
      });
      setOrders(
            ...
            <table className={tableClasses.table}>
              ...
              <tbody>
                {orders.map((order) => {
                  ...
                  const payMethod = {
                    class: "bg-amber-900/50 border-amber-700 text-amber-300",
                  ...
                  return (
                    <tr key={order._id} className={tableClasses.row}>
                      ...
                      <td className={tableClasses.cellBase}>
                        <div className="text-amber-100 text-sm max-w-[200px]">
                      ...
                      <td className={tableClasses.cellBase}>
                        <div className="space-y-1 max-h-52 overflow-auto">
                          {order.items.map((itm, idx) => (
                            ...
                            >
                              <img
                                src={`https://mern-food-delivery-webapplication-backend.onrender.com${itm.item.imageUrl}`}
                                alt={itm.item.name}
                              ...
                              <div className="flex-1">
                                ...
                                <div className="flex items-center gap-2 text-xs text-amber-400/50">
                                  <span>Rs{itm.item.price.toFixed(2)}</span>
                                  <span>&dot;</span>
