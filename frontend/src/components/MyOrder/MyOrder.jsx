...
import { FiArrowLeft, FiBox, FiMapPin, FiUser, FiClock, FiTruck, FiCheckCircle } from "react-icons/fi";
...
const MyOrder = () => {
  ...
  useEffect(() => {
    const fetchOrders = async () => {
      try {
        const response = await axios.get("https://mern-food-delivery-webapplication-backend.onrender.com/api/orders", {
  ...
  if (error)
    return (
      <div className="min-h-screen bg-gradient-to-br from-[#1a120b] via-[#2a1e14] to-[#3e2b1d] flex items-center justify-center text-red-600 text-xl gap-4">
        <p>{error}</p>
        <button
                    ...
                    <tr key={order._id} className="border-b border-amber-500/20 hover:bg-[#3a2b2b]/30 transition-colors group">
                      ...
                      <td className="p-4">
                        <div className="flex items-center gap-2">
                          ...
                          <div>
                            <p className="text-amber-100/80 text-sm max-w-[200px]">{order.address}, {order.city}-{order.zipCode}</p>
                          </div>
                        </div>
                      </td>
                        ...
                        <div className="space-y-2">
                          {order.items.map((item,index) => (
                            <div key={`${order._id}-${index}`} className="flex items-center gap-3 p-2 bg-[#3a2b2b]/50 rounded-lg">
                              <img 
                                src={`https://mern-food-delivery-webapplication-backend.onrender.com${item.item.imageUrl}`} 
                                alt={item.item.name} 
                      ...
                      <td className="p-4 text-amber-300 text-lg">Rs.{totalPrice.toFixed(2)}</td>
                      <td className="p-4">
                        <div className="flex flex-col gap-2">
                          <div className={`${paymentMethod.class} px-3 py-1.5 rounded-lg border text-sm`}>
                            {paymentMethod.label}
                          </div>
                          <div className={`${paymentStatus.color} px-3 py-1.5 rounded-lg text-sm flex items-center gap-2`}>
                            {paymentStatus.icon}
