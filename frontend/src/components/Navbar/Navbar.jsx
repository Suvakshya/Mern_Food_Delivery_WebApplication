...
const Navbar = () => {
  ...
  const renderMobileAuthButton = () =>{
    return isAuthenticated ? (
      <buton onClick={handleLogout} className='px-3 lg:px-4 py-1.5 lg:py-2 bg-gradient-to-br from-amber-500 to-amber-700 text-[#2D1B0E] rounded-xl font-semibold flex items-center justify-center space-x-2 text-sm'>
        <FiLogOut/>
        <span>Logout</span>
      </buton>
    ) :(
      <buton onClick={()=>{
        navigate('/login')
        setIsOpen(false)
      }} className='px-3 lg:px-4 py-1.5 lg:py-2 bg-gradient-to-br from-amber-500 to-amber-700 text-[#2D1B0E] rounded-xl font-semibold flex items-center justify-center space-x-2 text-sm'>
        <FiKey/>
        <span>Login</span>
      </buton>
    )
  ...
  return (
    <nav className="bg-[#2D1B0E] border-b-8 border-amber-900/30 shadow-amber-900/30 sticky top-0 z-50 shadow-[0_25px_50px_-12px] font-vibes group/nav overflow-x-hidden">
      <div className="absolute top-3 left-1/2 -translate-x-1/2 w-full max-w-7xl px-4 ">
        <div className="h-[6px] bg-gradient-to-r from-transparent via-amber-600/50 to-transparent shadow-[0_0_0_20px] shadow-amber-500/30" />
              ...
              <NavLink
                ...
                className={({ isActive }) =>
                  `group px-3 md:px-3 lg:px-4 py-2 lg:py-3 text-sm md:text-[15px] lg:text-base relative transition-all duration-300 flex items-center hover:bg-amber-900/20 rounded-3xl border-2 ${
                    isActive
                      ? "border-amber-600/50 bg-amber-900/20 shadow-[inner_0_0_15px] shadow-amber-500/20"
      ...
      {showLoginModel && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
          <div className="bg-gradient-to-br from-[#2D1B0E] to-[#4a372a] rounded-xl p-6 w-full max-w-[400px] relative border-4 border-amber-700/30 shadow-[0_0_30px] shadow-amber-500/30">
            <button
              onClick={() => navigate("/")}
